...
const getNameOptions = () => {
	...
	const options = [ { label: 'Date', value: `image-${ date }` } ];
...
function Plugin() {
	...
	useEffect( () => {
		const onPaste = ( event ) => {
			...
			const url =
				urlFromHtml ||
				( isLikelyImageUrl( plainText ) ? plainText : '' );
	...
	return (
		<>
			{ isOpen && (
				<Modal title="Name pasted image" onRequestClose={ closeModal }>
...
registerPlugin( 'block-editor-image-filename-prompt', { render: Plugin } );
